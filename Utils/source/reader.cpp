...
Tuple Reader::read()
{
    ...
    if (version == 4)
    {
        ...
        Reader::byteOrderSwap(&ipv4Header.sourceIP, sizeof(ipv4Header.sourceIP));
        Reader::byteOrderSwap(&ipv4Header.destIP, sizeof(ipv4Header.destIP));
        tuple.version = 4;
        tuple.srcAddr = Reader::binaryToString(ipv4Header.sourceIP);
        tuple.dstAddr = Reader::binaryToString(ipv4Header.destIP);
    ...
    else if (version == 6)
    {
        ...
        tuple.version = 6;
        tuple.srcAddr = Reader::binaryToString(ipv6Header.source_ipv6_high, ipv6Header.source_ipv6_low);
        tuple.dstAddr = Reader::binaryToString(ipv6Header.dest_ipv6_high, ipv6Header.dest_ipv6_low);
    ...
    if (tuple.protocol == 6 || tuple.protocol == 17)
    {
        ...
        Reader::byteOrderSwap(&tuple.srcPort, sizeof(tuple.srcPort));
        Reader::byteOrderSwap(&tuple.dstPort, sizeof(tuple.dstPort));
    }
    else
    {
        tuple.srcPort = 0;
...
std::vector<Tuple> Reader::read(std::size_t number)
{
...
void Reader::skip(std::size_t offset)
{
...
std::string Reader::binaryToString(uint32_t ip) noexcept
{
...
std::string Reader::binaryToString(uint64_t ipv6_high, uint64_t ipv6_low) noexcept
{
    uint16_t group[8] = {0};
...
void Reader::byteOrderSwap(void *data, size_t size) noexcept
{
...
uint32_t toBinary(const std::string &ipv4) noexcept
{
    uint32_t result = 0;
    size_t pos = 0;
    for (int i = 0; i < 4; i++)
    {
        result <<= 8;
        result |= std::stoi(ipv4.substr(pos, ipv4.find_first_of('.', pos) - pos));
        pos = ipv4.find_first_of('.', pos) + 1;
    }
    return result;
}
